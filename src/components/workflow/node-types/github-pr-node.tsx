'use client';

import { memo } from 'react';
import { Handle, Position, type NodeProps } from '@xyflow/react';
import { GitPullRequest } from 'lucide-react';
import { cn } from '@/lib/utils';

interface GithubPrNodeData {
  label: string;
  prTitle?: string;
  prTitleTemplate?: string;
  baseBranch?: string;
  draft?: boolean;
  isRunning?: boolean;
  isCompleted?: boolean;
  isFailed?: boolean;
  [key: string]: unknown;
}

function GithubPrNode({ data, selected }: NodeProps & { data: GithubPrNodeData }) {
  return (
    <div
      className={cn(
        'relative rounded-md border border-border bg-card min-w-[180px] shadow-sm transition-shadow',
        selected && 'node-glow-blue',
        data.isCompleted && 'border-emerald-500/50',
        data.isFailed && 'border-rose-500/50'
      )}
    >
      <div className="h-[2px] w-full rounded-t-md bg-blue-500" />

      <Handle type="target" position={Position.Top} className="!bg-handle !w-2.5 !h-2.5 !border-2 !border-card !ring-1 !ring-border" />

      <div className="px-2.5 py-2">
        <div className="flex items-center gap-1.5">
          <div className="flex items-center justify-center w-5 h-5 rounded-full bg-blue-500/15">
            <GitPullRequest className="w-3 h-3 text-blue-400" />
          </div>
          <p className="text-sm font-medium text-foreground">{data.label || 'GitHub PR'}</p>
          {data.draft && (
            <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-blue-500/15 text-blue-400">draft</span>
          )}
        </div>
        {(data.prTitle || data.prTitleTemplate) && (
          <p className="text-[11px] text-dim mt-0.5 line-clamp-1">
            {data.prTitleTemplate ? '{{template}}' : data.prTitle}
          </p>
        )}
        {data.baseBranch && (
          <p className="text-[10px] text-dim mt-0.5 font-mono">→ {data.baseBranch}</p>
        )}
      </div>

      <Handle type="source" position={Position.Bottom} className="!bg-handle !w-2.5 !h-2.5 !border-2 !border-card !ring-1 !ring-border" />
    </div>
  );
}

export default memo(GithubPrNode);
